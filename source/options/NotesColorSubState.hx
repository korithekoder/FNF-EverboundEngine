...
class NotesColorSubState extends MusicBeatSubstate
{
	...
	public function new() {
		...
		copyButton = new FlxSprite(760, 50).loadGraphic(PathsUtil.image('noteColorMenu/copy'));
		copyButton.alpha = 0.6;
		add(copyButton);

		pasteButton = new FlxSprite(1180, 50).loadGraphic(PathsUtil.image('noteColorMenu/paste'));
		...
		colorPalette = new FlxSprite(820, 580).loadGraphic(PathsUtil.image('noteColorMenu/palette', false));
		colorPalette.scale.set(20, 20);
		colorPalette.updateHitbox();
		colorPalette.antialiasing = false;
		add(colorPalette);
		
		colorWheel = new FlxSprite(860, 200).loadGraphic(PathsUtil.image('noteColorMenu/colorWheel'));
		...
		tip.setFormat(PathsUtil.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tip.borderSize = 2;
		add(tip);

		tipTxt = new FlxText(tipX, tipY + 24, 0, '', 16);
		tipTxt.setFormat(PathsUtil.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tipTxt.borderSize = 2;
		add(tipTxt);
		updateTip();
	...
	public function spawnNotes()
	{
		...
		skinNote = new FlxSprite(48, 24).loadGraphic(PathsUtil.image('noteColorMenu/' + (onPixel ? 'note' : 'notePixel')), true, res, res);
		...
		for (i in 0...3)
		{
			var newNote:FlxSprite = new FlxSprite(230 + (100 * i), 100).loadGraphic(PathsUtil.image('noteColorMenu/' + (!onPixel ? 'note' : 'notePixel')), true, res, res);
